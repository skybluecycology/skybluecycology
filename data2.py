...
from pyspark.sql.functions import col, expr, lit, concat_ws
...
# Add descriptive columns for comparison results
for col_name in columns_to_compare:
    differences = differences.withColumn(f"diff_{col_name}", 
                                         concat_ws(" -> ", col(f"df1.{col_name}"), col(f"df2.{col_name}")))

# Select key columns and differences columns
select_columns = key_columns + [f"diff_{col_name}" for col_name in columns_to_compare]
differences = differences.select(*select_columns)
